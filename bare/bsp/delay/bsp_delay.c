...
{
	GPT2->CR = 0; 					/* 清零，bit0也为0，即停止GPT  			*/

	GPT2->CR = 1 << 15;				/* bit15置1进入软复位 				*/
	while((GPT2->CR >> 15) & 0x01);	/*等待复位完成 						*/
	 ...
	 */
	GPT2->PR = 65;	/* 设置为65，即66分频，因此GPT2时钟为66M/(65+1)=1MHz */
     ...
     *          以此类推，最大可以设置为0XFFF，也就是最大4096分频
	 */
	GPT2->PR = 65;	//设置为1，即65+1=66分频，因此GPT2时钟为66M/66=1MHz
	 ...
	 */
	GIC_EnableIRQ(GPT2_IRQn);	//使能GIC中对应的中断
	system_register_irqhandler(GPT2_IRQn, (system_irq_handler_t)GPT2_irqhandler, NULL);	//注册中断服务函数	
#endif
